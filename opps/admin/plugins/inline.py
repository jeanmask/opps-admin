...
from opps.admin.layout import FormHelper, Layout, flatatt, Container, Column, Field, Fieldset
from opps.admin.sites import site
from opps.admin.views import BaseAdminPlugin, ModelFormAdminView, DetailAdminView, filter_hook


class ShowField(Field):
    template = "opps/admin/layout/field_value.html"
...
class TDField(Field):
    template = "opps/admin/layout/td-field.html"
...
class InlineStyle(object):
    template = 'opps/admin/edit_inline/stacked.html'
...
class InlineFormset(Fieldset):
    ...
    def __init__(self, formset, allow_blank=False, **kwargs):
        ...
        if allow_blank and len(formset) == 0:
            self.template = 'opps/admin/edit_inline/blank.html'
        self.formset = formset
